...
export class AppComponent implements OnInit {
  ...
  url: any = "https://firebasestorage.googleapis.com/v0/b/morbargig-a81d2.appspot.com/o/CV%2Fno-photo-available.png?alt=media&token=27b382af-7a35-4551-ade9-5edb5271df6b"
  buttonsText: { eName: string, hName: string, name: string }[] = [
  ...
  ngOnInit() {
    this.PdfApi.getPdf().valueChanges().subscribe(data => {
      this.pdf = data
      this.saveUrl(this.pdf[this.pdf.language])
    })
  }
  newPdf() {
    this.PdfApi.newPdf()
  ...
  admin() {
    ...
    if (this.PdfApi.verifyAdmin(Password)) {
      this.changePdf = true;
    }
    else {
      alert("sorry you are not authorized to edit files");
    }
  }
  updatePdf() {
